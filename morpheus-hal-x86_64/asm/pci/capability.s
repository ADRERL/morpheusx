...
; CL = bus, DL = device, R8B = function. Returns EAX = 1 if capabilities
...
; CL = bus, DL = device, R8B = function. Returns EAX = capability
; pointer (offset), or 0 if no capabilities.
asm_pci_get_cap_ptr:
    push    rcx
    push    rdx
    push    r8
    ...
    and     eax, 0xFC            ; dword-align (bottom 2 bits must be 0)
    jmp     .done
...
; CL = bus, DL = device, R8B = function, R9B = capability offset.
; Returns AL = capability ID, AH = next capability offset (0 = end).
asm_pci_read_cap_header:
    call    asm_pci_cfg_read16
    ret

; CL = bus, DL = device, R8B = function, R9B = capability ID to find.
; Returns EAX = offset of capability, or 0 if not found.
asm_pci_find_cap:
    push    rcx
    push    rdx
    push    r8
    ...
    movzx   r10d, r9b            ; target cap ID

    push    r9                   ; asm_pci_get_cap_ptr clobbers r9
    ...
    mov     r11d, eax            ; current capability offset
    mov     r12d, 256            ; safety counter (max iterations)
...
.walk_loop:
    ...
    movzx   r9d, r11b            ; movzx to avoid partial-register write
    ...
    shr     eax, 8               ; next pointer from AH
    and     eax, 0xFC
    mov     r11d, eax
    jmp     .walk_loop
...
; CL = bus, DL = device, R8B = function, R9B = starting capability offset
; (search begins after this), R10B = capability ID to find.
; Returns EAX = offset of next matching capability, or 0 if not found.
asm_pci_find_cap_next:
    ...
    movzx   r13d, r10b           ; target cap ID
    movzx   r11d, r9b            ; current offset
...
; VirtIO caps are vendor-specific (ID=0x09) with an additional cfg_type
; field. Layout: [cap_vndr:8][cap_next:8][cap_len:8][cfg_type:8][bar:8][...]
;
; CL = bus, DL = device, R8B = function, R9B = VirtIO cfg_type to find
; (1=common, 2=notify, 3=isr, 4=device).
; Returns EAX = offset of capability, or 0 if not found.
asm_pci_find_virtio_cap:
    ...
    movzx   r13d, r9b            ; target cfg_type
    ...
    mov     r11d, eax            ; current vndr cap offset
