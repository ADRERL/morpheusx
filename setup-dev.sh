...
do_create_disk() {
    ...
    #   p2: HelixFS data partition (raw, formatted by helix host_tool)
...
cmd_flash() {
    ...
    printf "           copy ESP files + pack helix.img. No full dd.\n"
    printf "\n"
    ...
    # build the standalone helix host tool (lives outside the workspace)
    (cd "${PROJECT_ROOT}/helix/host_tool" && cargo build --release 2>&1) \
        || die "helix host_tool build failed"
    sudo "${PROJECT_ROOT}/helix/host_tool/target/release/host_tool" format "$helix_part" \
    ...
    if ! sudo "${PROJECT_ROOT}/helix/host_tool/target/release/host_tool" pack "$helix_part" "$mnt/morpheus/helix.img" --max-mb "$payload_mb"; then
        sudo umount "$mnt" || true
        rmdir "$mnt" || true
...
usage() {
    ...
    printf "  %s\n\n" "$(basename "$0")"
    printf "  ${C_DIM}# Force full rebuild + launch${C_RESET}\n"
    printf "  %s -f\n\n" "$(basename "$0")"
    ...
    printf "  %s thinkpad\n\n" "$(basename "$0")"
    printf "  ${C_DIM}# Flash to real SD card / USB stick (no full dd)${C_RESET}\n"
