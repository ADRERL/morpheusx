//! host-side helixfs tool: injects binaries into helix-data.img so they exist
//! when morpheusx boots in qemu.
...
// mounts the disk's helixfs, formatting when there is no valid superblock;
// HelixFs::mount does recovery, log replay, and bitmap rebuild internally
fn mount(disk: &str) -> Result<(FileBlockDevice, HelixFs), String> {
    let mut dev =
        FileBlockDevice::open(disk).map_err(|e| format!("cannot open '{}': {}", disk, e))?;

    println!(
        "[helix] disk: {} sectors x {} bytes",
...
fn cmd_inject(disk: &str, binary: &str, dest: &str) -> Result<(), String> {
    ...
    if elf_bytes.len() < 4 || &elf_bytes[0..4] != b"\x7fELF" {
        // data files (.kmap layouts etc) get injected too
        println!(
    ...
    // mkdir -p every parent: write() auto-creates them too, but explicit
    // mkdirs surface per-dir errors and keep the log tidy
    {
    ...
    println!("[inject] wrote {} bytes to {}", written, dest);
    println!(
...
// depth-first: unlink only removes files or empty dirs. inject overwrites by
// path but never deletes, so clearing a stale tree needs this
